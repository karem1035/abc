...
test('requires both bodies for publication',()=>{expect(postInput.safeParse({slug:'test',type:'article',titleAr:'عنوان',titleEn:'Title',contentAr:'<script>x</script>',contentEn:'<p>Text</p>',status:'published',commentsEnabled:true}).success).toBe(false)})
describe.skipIf(process.env.RUN_POST_INTEGRATION!=='1')('local API integration',()=>{
 test('roles, publication, translations, search and comment moderation',async()=>{
 ...
 const body={slug:`test-${suffix}`,type:'article',titleAr:'عنوان الاختبار',titleEn:`Integration ${suffix}`,contentAr:'<p>محتوى عربي</p>',contentEn:'<h2>English</h2><script>bad()</script>',status:'draft',commentsEnabled:true}
 ...
 expect((await req(`/${body.slug}/comments`,'POST',{name:'Reader',body:'Useful article',locale:'en'})).status).toBe(201)
 expect((await (await req(`/${body.slug}/comments?locale=en`)).json()).total).toBe(0)
 const queue=await (await req('/admin/comments?status=pending&limit=50','GET',undefined,token)).json();const comment=queue.data.find((c:{postId:string})=>c.postId===postId);expect(comment).toBeDefined()
 expect((await req(`/admin/comments/${comment.id}`,'PATCH',{status:'approved'},token)).status).toBe(200)
 expect((await (await req(`/${body.slug}/comments?locale=en`)).json()).total).toBe(1);expect((await (await req(`/${body.slug}/comments?locale=ar`)).json()).total).toBe(0)
 await req(`/admin/${postId}`,'PUT',{...body,status:'published',commentsEnabled:false},token)
 expect((await req(`/${body.slug}/comments`,'POST',{name:'Reader',body:'Comment again',locale:'en'})).status).toBe(404)
