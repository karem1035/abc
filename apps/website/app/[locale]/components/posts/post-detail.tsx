...
export async function PostDetail({locale,slug,type}:{locale:Locale;slug:string;type:'article'|'news'}) {
 ...
 return <div className="hospital-directory"><article className="post-reading"><nav className="post-breadcrumb"><Link href={`/${locale}`}>{ar?'الرئيسية':'Home'}</Link><span>/</span><Link href={base}>{type==='news'?(ar?'الأخبار':'News'):(ar?'المدونة':'Blog')}</Link></nav><header><p className="post-kicker">{post.category&&<Link href={`${base}?category=${encodeURIComponent(post.category)}`}>{post.category}</Link>}</p><h1>{post.title}</h1><p className="post-standfirst">{post.excerpt}</p><div className="post-byline">{post.author&&<span>{post.author}</span>}<time dateTime={post.publishedAt??undefined}>{postDate(post.publishedAt,locale)}</time></div></header>{post.coverUrl&&<img className="post-reading-cover" src={post.coverUrl} alt={post.title}/>}<div className="prose-hospital post-prose" dangerouslySetInnerHTML={{__html:post.content??''}}/></article><LatestPosts locale={locale} exclude={post.id}/></div>
}
