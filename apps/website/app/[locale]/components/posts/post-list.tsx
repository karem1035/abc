...
export async function PostList({locale,type,search}:{locale:Locale;type:'article'|'news';search:{q?:string;page?:string;category?:string}}) {
 ...
 // distinct localized categories for the filter chips (blog only)
 ...
 {categories.length>0&&<nav className="post-category-chips" aria-label={ar?'تصفية حسب التصنيف':'Filter by category'}>
  <Link href={categoryHref()} aria-current={!category?'page':undefined}>{ar?'كل التصنيفات':'All topics'}</Link>
  {categories.map(cat=><Link key={cat} href={categoryHref(cat)} aria-current={category===cat?'page':undefined}>{cat}</Link>)}
 </nav>}
 {result.data.length?(type==='news'
  ?<div className="news-list">{result.data.map(post=><NewsCard key={post.id} post={post} locale={locale}/>)}</div>
  :<div className="post-grid">{result.data.map(post=><ArticleCard key={post.id} post={post} locale={locale}/>)}</div>
 ):<div className="hospital-directory-empty"><BookOpen size={35}/><h2>{q?(ar?'لا توجد نتائج':'No matching posts'):(ar?'لا توجد منشورات حالياً':'No posts published yet')}</h2><p>{ar?'تصفح الأخبار أو عد قريباً للاطلاع على جديدنا.':'Explore our news or check back for updates.'}</p>{q&&<Link href={base}>{ar?'مسح البحث':'Clear search'}</Link>}</div>}
